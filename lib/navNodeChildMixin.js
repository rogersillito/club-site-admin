...
var addNavNodeChildBehaviours = function(ListDefinition) {
  ...
  ListDefinition.schema.methods.getAllDescendentNodes = function() {
    var startNode = this;
    var recurse = function(parent, promises, nodes) {
      // stop condition = no more children

      // return new Promise(function(resolve, reject) {
      // });

        NavNode.model.find({parent: parent._id}).exec(function(err, descendents) {
          if (err) {
            promises.push(Promise.reject(err));
            return promises;
          }
          _.each(descendents, function(descendent) {
            promises.push(Promise.resolve(descendent));
          });
          return promises;
        });
    };
    return Promise.all(recurse(startNode,[],[]));
  };
