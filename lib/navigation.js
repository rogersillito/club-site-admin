...
exports.getMenuLinkMatching = function(requestUrl) {
  return new Promise(function (resolve, reject) {
    keystone.list('MenuLink').model.find()
      .where('isPublished').equals(true)
      .exec(function(err, links) {
      if (err) {
        reject(err);
      }
      var sorted = _.sortBy(links, function(link) { return 1 / link.relativeUrl.length; });
      var match = _.find(sorted, function(link) { return requestUrl.startsWith(link.relativeUrl); });
      if (match) {
        return resolve(match);
      }
      return reject(`No MenuLink matches: ${requestUrl}`);
    });
  });
};
