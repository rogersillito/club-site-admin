...
var getFilterCriteriaFor = function(monthName, year) {
  ...
  dateHelpers.validateYear(year);
	const setMidnight = m => m.set('h',0).set('m',0).set('s',0).set('ms',0);
  var filterStartDate = setMidnight(moment()
        .year(year)
        .month(monthIdx)
        .date(1));
  var filterEndDate = setMidnight(moment()
        .year(year)
        .month(monthIdx + 1)
        .date(1));
  return {
    ...
    q.exec(function(err, results) {
      ...
      const mapped = _.map(results, function(r) {
        ...
        const output = {
          ...
          resultDate: new moment(r.date).format(dateHelpers.formatStrings.dayDateString),
          link: `/results/${m}/${r.year}#${r.key}`,
          hasResultsContent: !!(r.resultHtml && r.resultHtml.trim()),
          fullResultsLink: r.resultUrl ? r.resultUrl : undefined
        };
